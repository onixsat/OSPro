...
namespace Compact_RAM_Cleaner
{
    public static class UpdateSystem
    {
        ...
        {
            if (MessageBox.Show(Translations.GetString("UpdateAvailable"), "OS RAM Cleaner", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    var temp = $"{Path.GetTempPath()}\\OS RAM Cleaner.exe";
                    ...
                    Cmd($"taskkill /f /im \"{ExeName}\" & del \"{Paths.ApplicationExe}\" & move \"{temp}\" \"{Paths.ApplicationDirectory}\" & \"{Paths.ApplicationDirectory}\\OS RAM Cleaner.exe\"");
                    Environment.Exit(0);
                }
