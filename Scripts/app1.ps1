...
if ((CheckAdmin) -eq $false)
{
	...
	else
	{
		...
		if ($IsCoreCLR) { $PowerShellCmdLine = "pwsh.exe" }
		else { $PowerShellCmdLine = "powershell.exe" }
...
Function Cleanup
{
	...
	$DeleteOldDownloads = Read-Host "Gostaria de excluir arquivos com mais de 90 dias na pasta Downloads para Todos os Usuários? (Y/N)" -Default "N"
	if (-not ($DeleteOldDownloads))
	{
	...
	$CleanBin = Read-Host "Gostaria de esvaziar a Lixeira de Reciclagem para Todos os Usuários? (Y/N)"
