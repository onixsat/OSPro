...
namespace Compact_RAM_Cleaner
{
    public partial class Settings : FormWithShadow
    {
        ...
        void InitializeForm()
        {
            ...
            {
                if (MessageBox.Show(Translations.GetString("ResetSettings") + "?", "OS RAM Cleaner", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
            ...
            AutorunCheck.Click += (s, e) =>
            {
                using (var key = Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\Windows\CurrentVersion\Run"))
                {
                    if (AutorunCheck.Checked)
                        key.SetValue("OS RAM Cleaner", $"\"{Paths.ApplicationExe}\" silent");
                    else
                        key.DeleteValue("OS RAM Cleaner", false);
                }
            };
        ...
        void InitializeTab2()
        {
            ...
            {
                _trayIcon.OnMiddleMouseClick = () => Process.Start("notepad.exe");
                Save("MiddleMouseClick", "notepad.exe");
            };
        }
        #endregion
        ...
        void CheckSettings()
        {
            ...
            {
                if (key?.GetValue("OS RAM Cleaner") != null)
        ...
        private void AboutLink4_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
        ...
        private void TabPanel3_Paint(object sender, PaintEventArgs e)
        {
